  ...
  aplicaciones.setProperty(x, "minimizado", true)
   ...
   FontLoader {
    ...
    GridView { //Muestra los programas en la barra de tareas
        ...
        cellHeight: 40
        cellWidth: 62
        z: 20; width: screenWidth; height: 40 
        model: aplicaciones
        delegate: tareas
    }
...
Image {
    ...
    Keys.onSpacePressed: {abrirapp("Worms", ""); menu.state = "cerrado";}
    ...
    MouseArea{
        ...
        onClicked: {
            menu.state = "cerrado";
            abrirapp("Explorador", "/home/dante")
        }
    }}
 ...
 Component { //Modelo de como se muestran los
    ...
    ListView {
        x: 13; y: 13; width: 228; height: 222
        clip: true
        FolderListModel {
            id: instaladas
...
function wnivel(x){
      if (x == 1) {return "graficos/wifi_5.png"}

else if (x == 2) {return "graficos/wifi_100.png"}
}
...
else if (x == 3) {return "graficos/bateria.png"}
}
...
Image {
  ...
  Image {
    ...
    x: parent.width
    width: screenWidth -39 - 214 - 15 -95
    Image {
...
//Dialogos del menu
    ...
    BorderImage { //Bateria
        id: bateria
        x: screenWidth -width -3; y: -height
    ...
    BorderImage { //Volumen
        id: volumen
        x: screenWidth -width -81; y: -height
        width: 70; height: 291
        border { left: 1; top: 1; right: 1; bottom: 1 }
        horizontalTileMode: BorderImage.Stretch
        verticalTileMode: BorderImage.Stretch
    ...
    BorderImage { //Wifi menu
        id: wland
        x: screenWidth -width -31; y: -height
    ...
    BorderImage { //Mas botones de la barra
        id: textra
        x: screenWidth -width -178; y: -height
        width: 106; height: 91
        border { left: 1; top: 1; right: 1; bottom: 1}
        horizontalTileMode: BorderImage.Stretch
        verticalTileMode: BorderImage.Stretch
    ...
    //Text {text: pantainfo.screen +" "+pantainfo.physicalWidth; y: -100; color: "red"; visible: false}

      
    function menua() {
        if (menu.state == "cerrado"){menu.state = "abierto"; menu.forceActiveFocus()}
        else {menu.state = "cerrado"};
    }

    Component.onCompleted: forceActiveFocus();
    Keys.onSpacePressed: menua();
