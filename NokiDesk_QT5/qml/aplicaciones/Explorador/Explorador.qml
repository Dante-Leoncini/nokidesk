...
function volver(x){
    if (x == 1 && 0 < indice){
        indice--
        folderModel.folder = Logic.lista[indice];
        laurl.text = ncarpeta(Logic.lista[indice])
    }
    else if (x == 2 && Logic.lista.length > indice){
        indice++
        folderModel.folder = Logic.lista[indice];
        laurl.text = ncarpeta(Logic.lista[indice])
    }
...
function flechas(x){
    if (enfocadot()) {
        ...
        else if (x == 1 && indice == 1) {return "ATRAS_I.PNG"}
...
function activo(x){
    ...
    Component.onCompleted: {
        //parent.parent.fichero = "/home/dante"
        //folderModel.source = "/home/dante"
        Ui.scrollBar(tema, visor, grid, Qt.Vertical) //crea la Barra de Scroll
        Logic.lista[0] = folderModel.folder
        parent.parent.nombre = "" //parent.parent.fichero
    }
...
else if (extension(z) == "txt" || extension(z) == "js") {return "../../temas/"+tema+"/iconos/txt.png"}
...
function ejecutar(x,y) {
    ...
    abrirapp("Bloc de notas", "file:///" + x, y, false);
    }

    else if (extension(x) == "png" || extension(x) == "PNG" || extension(x) == "jpg" || extension(x) == "bmp" || extension(x) == "svg" || extension(x) == "gif"){
    abrirapp("Visualizador de Fotos", "file:///" + x, y, false);
    }

    else if (extension(x) == "AVI" || extension(x) == "avi" || extension(x) == "mp3" || extension(x) == "mp4" || extension(x) == "wmv" || extension(x) == "3gp" || extension(x) == "mkv"){
    abrirapp("Reproductor", "file:///" + x, y, false);
    }

    else {       
        indice++
        if (x == undefined) {Logic.lista.push("root")}
        else {Logic.lista.push("file://"+x)}
        folderModel.folder = Logic.lista[indice]
        laurl.text = y
    }
}
    ...
    Rectangle {
        ...
        y: 64; z: parent.z - 1
        width: parent.width
        height: parent.parent.height -38 -24 - detalles.height
        color: "white"
        ...
        GridView {
            ...
            FolderListModel {
                id: folderModel
                folder: {source: "file:///"}//carpeta(aplicaciones.get(cerrar(indice)-1).fichero)}
                //nameFilters: ["*.qml"]
            }
